module Analytics
  module Models
    require_dependency 'analytics/models/analytics'

    require_dependency 'analytics/models/simple_analytics'
    require_dependency 'analytics/models/google_analytics'
    require_dependency 'analytics/models/classic_google_analytics'
    require_dependency 'analytics/models/universal_google_analytics'

    require_dependency 'analytics/models/advanced_analytics'
  end
end

